...
export default function CartPage() {
  ...
  const updateQty = async (item, newQty) => {
    ...
    try {
      await fetch(`${API_BASE}/carts/update/${userId}`, {
        method: "PATCH",
  ...
  return (
    <div style={{ padding: "20px 5%", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 20 }}>
            ...
            <div key={item.product_id} style={{ display: "flex", gap: 15, padding: 10, border: "1px solid #f0f0f0", borderRadius: 10, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 150 }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                  <div style={{ display: "flex", border: "1px solid #e0e0e0", borderRadius: 6, alignItems: "center" }}>
                    <button onClick={() => updateQty(item, item.quantity - 1)} style={{ padding: "4px 10px", border: "none", background: "none", cursor: "pointer" }}>-</button>
                    <div style={{ padding: "4px 12px", borderLeft: "1px solid #eee", borderRight: "1px solid #eee" }}>{item.quantity}</div>
                    <button onClick={() => updateQty(item, item.quantity + 1)} style={{ padding: "4px 10px", border: "none", background: "none", cursor: "pointer" }}>+</button>
                  </div>
                  <button onClick={() => removeItem(item)} style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "none", color: "#dc2626", cursor: "pointer", fontWeight: 500 }}>
        ...
        {/* Price Summary */}
        <div style={{ border: "1px solid #f8f8fb", borderRadius: 16, padding: 20, boxShadow: "0 2px 10px rgba(0,0,0,0.02)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 15 }}>
            <ShoppingCart size={20} color="#5d5ad1" />
            <h3 style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>Price Summary</h3>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, borderBottom: "1px solid #f0f0f0", paddingBottom: 15, fontSize: 14, color: "#666" }}>
          ...
          <button onClick={handleCheckout} style={{ width: "100%", padding: 14, background: "#4f46e5", color: "#fff", border: "none", borderRadius: 10, fontWeight: 600, fontSize: 15, cursor: "pointer", marginTop: 15, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            Proceed to Checkout <ArrowRight size={18} />
          </button>
          <div style={{ textAlign: "center", marginTop: 10 }}>
            <button onClick={() => navigate("/shop")} style={{ background: "none", border: "none", color: "#5d5ad1", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
              Continue Shopping
            </button>
          </div>
        </div>
